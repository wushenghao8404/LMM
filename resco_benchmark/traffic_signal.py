        ...
        for j in range(0, len(phases)):
            if i != j:
                ...
                for sig_idx in range(len(phases[i].state)):
                    if (phases[i].state[sig_idx] == 'G' or phases[i].state[sig_idx] == 'g') \
                            and (phases[j].state[sig_idx] == 'r' or phases[j].state[sig_idx] == 's'):
                        need_yellow = True
                        yellow_str += 'y'
                    else:
                ...
                # if not need_yellow:
                #     print(phases[i].state, phases[j].state, i, j)
...
class Signal:
    ...
    def prep_phase(self, new_phase):
        print(self.id, self.phases, self.phase, new_phase, self.yellow_dict)
