...
try:
    ...
    tf.disable_eager_execution()  # 禁用 eager execution
...
if tf is None:
    ...
    class FMA2C(Agent):
        def __init__(self, config, obs_act, map_name, thread_number):
            ...
            print(management, 'obs_act', obs_act)
            self.state = None
            self.acts = None
            ...
            for manager in management:
                ...
                # self.managers[manager].model.load(config['model_path'])
                ...
                for worker_id in worker_ids:
                    ...
                    # self.workers[worker_id].model.load(config['model_path'])
            ...
            if 'model_path' in config:
                if tf.train.checkpoint_exists(config['model_path']):
                    ...
                    print("Model loaded successfully from: {}".format(config['model_path']))
