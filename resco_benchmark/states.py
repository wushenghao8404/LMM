...
def gplight(signals):
    """
        Reference: Learning Traffic Signal Control via Genetic Programming, GECCO 2024.
    """
    observations = dict()
    # aa = []
    for i, signal_id in enumerate(signals):
        ...
        state = [i]
        # print(signal.lane_sets)
        # a = np.sort([len(signal.lane_sets[direction]) for direction in signal.lane_sets])
        # aa.append(a[-2] + a[-1])

        for direction in signal.lane_sets:
            direction_state = []
            for lane in signal.lane_sets[direction]:
                direction_state += [signal.full_observation[lane]['queue'], signal.full_observation[lane]['approach']]
            state.append(direction_state)
        observations[signal_id] = np.asarray(state, dtype=object)
    # print(observations)
    # print(np.max(aa))
    return observations


def gplight_norm(signals):
    """
        Reference: Learning Traffic Signal Control via Genetic Programming, GECCO 2024.
    """
    observations = dict()
    ...
    for i, signal_id in enumerate(signals):
        ...
        for direction in signal.lane_sets:
            direction_state = []
            for lane in signal.lane_sets[direction]:
                direction_state += [signal.full_observation[lane]['queue'] / 28,
...
def timestep(signals):
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        observations[signal_id] = np.asarray([signal.sumo.simulation.getTime()/signal.sumo.simulation.getEndTime()])
    return observations
